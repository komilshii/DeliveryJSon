...
const toggleModalAuth = function() {
	loginInput.style.borderColor = '';//удалить
...
function notAuthorized() {
	...
	function logIn(event) {
		...
		if (valid(loginInput.value)){
			...
			logInForm.reset();//очищает поле до первоночальнх данных
			checkAuth();
		} else {
...
function init() {
	...
	inputSearch.addEventListener('keydown', function(event) {
		...
		if(event.keyCode === 13) {
			...
			const value = target.value.toLowerCase().trim();//принимает все запросы маленькими буквами
//если в начале или конце поставлен пробел убирает его
			target.value = '';
